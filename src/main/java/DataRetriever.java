...
public class DataRetriever {
    ...
    Order saveOrder(Order order) {
        try {
            ...
            if (existingOrder != null) {
                System.out.println("Cette commande existe déjà avec l'id : " + existingOrder.getId());
    ...
    public Double getDishCost(Integer dishId) {
        ...
        String sql = """
        SELECT SUM(di.quantity_required * i.price) AS cost
    ...
    public Double getGrossMargin(Integer dishId) {
        ...
        String sql = """
        SELECT d.selling_price - SUM(di.quantity_required * i.price) AS margin
        FROM dish d
    ...
    public void getStockStatistics(String periodicity, Instant min, Instant max) {
        // Injecte directement la périodicité dans le SQL
        String sql = """
        SELECT
            sm.id_ingredient,
            DATE_TRUNC('%s', sm.creation_datetime) AS period,
            SUM(
                CASE
        ...
        """.formatted(periodicity.toLowerCase());

        try (Connection conn = new DBConnection().getConnection();
             PreparedStatement ps = conn.prepareStatement(sql)) {

            // Seuls les 2 paramètres de date restent
            ps.setTimestamp(1, Timestamp.from(min));
            ps.setTimestamp(2, Timestamp.from(max));
            ...
            while (rs.next()) {
                ...
                // Récupère l'ingrédient pour l'affichage
                Ingredient ing = findIngredientById(ingredientId);

                System.out.println(
                        ing.getName() + " | " +
                                period.toLocalDateTime().toLocalDate() +
                                " → " + qty
                );
            }
